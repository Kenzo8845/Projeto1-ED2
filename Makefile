...
# ---------------------------------------------------------------------------
# Detecta o sistema operacional para comandos de limpeza e execução
# ---------------------------------------------------------------------------
ifeq ($(OS), Windows_NT)
    RM      = del /Q /F
    RMDIR   =
...
test_parser_geo: test/test_parser_geo$(EXE)PROJ_NAME = ted
...
# ---------------------------------------------------------------------------
tstall: test_hash test_quadra test_habitante test_svg test_parser_geo test_parser_pm test_parser_qry

# ---------------------------------------------------------------------------
# Limpeza — compatível com Windows (del) e Linux (rm -f)
# ---------------------------------------------------------------------------
ifeq ($(OS), Windows_NT)
clean:
	-del /Q /F src\*.o src\ted.exe Unity\unity.o 2>nul
	-del /Q /F test\test_hash.exe test\test_quadra.exe test\test_habitante.exe 2>nul
	-del /Q /F test\test_svg.exe test\test_parser_geo.exe test\test_parser_pm.exe 2>nul
	-del /Q /F test\test_parser_qry.exe 2>nul
else
clean:
...
clean:
	$(RM) src/*.o src/$(PROJ_NAME) $(UNITY_OBJ)
	$(RM) test/test_hash test/test_quadra test/test_habitante
	$(RM) test/test_svg test/test_parser_geo test/test_parser_pm
endif
